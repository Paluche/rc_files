...
globalkeys = awful.util.table.join(
    ...
    -- Audacious music control
    awful.key({ }, "XF86AudioPlay", function () awful.util.spawn("audacious -u") end),
    ...
    -- Monitor control
    -- 0: Same image everywhere
    awful.key({ "Control", "Mod1" }, "à",
        function ()
            awful.util.spawn("xrandr --output VGA1 --auto --same-as LVDS1")
            naughty.notify({ title = "Monitor control",
            text = "Same image everywhere" })
        end
    ),
    -- 1: Laptop screen only.
    awful.key({ "Control", "Mod1" }, "&",
        function ()
            awful.util.spawn("xrandr --output VGA --off")
            awful.util.spawn("xrandr --output VGA --off")
            naughty.notify({ title = "Monitor control",
            text = "Laptop screen only" })
        end
    ),

    -- 2: VGA screen only.
    awful.key({ "Control", "Mod1" }, "é",
        function ()
            awful.util.spawn("xrandr --output LVDS1 --off")
            awful.util.spawn("xrandr --output LVDS1 --off")
            naughty.notify({ title = "Monitor control",
            text = "VGA screen only" })
        end
    ),

    -- 3: Split screen VGA right of laptop screen.
    awful.key({ "Control", "Mod1" }, "\"",
        function ()
            awful.util.spawn("xrandr --output LVDS1 --auto")
            awful.util.spawn("xrandr --output VGA1 --auto --right-of LVDS1")
            naughty.notify({ title = "Monitor control",
            text = "Splitted, VGA right of laptop" })
        end
    ),

    -- 4: Split screen VGA left of laprop screen.
    awful.key({ "Control", "Mod1" }, "'",
        function ()
            awful.util.spawn("xrandr --output LVDS1 --auto")
            awful.util.spawn("xrandr --output VGA1 --auto --left-of LVDS1")
            naughty.notify({ title = "Monitor control",
            text = "Splitted, VGA left of laptop" })
        end
    ),

    -- 5: Split screen, HDMI right of laptop screen.
    awful.key({ "Control", "Mod1" }, "(",
        function ()
            awful.util.spawn("xrandr --output HDMI1 --auto")
            awful.util.spawn("xrandr --output HDMI1 --auto --left-of LVDS1")
            naughty.notify({ title = "Monitor control",
            text = "Splitted, HDMI right of laptop" })
        end
    ),

    -- Touchpad activation toggle
    ---- Toggle original funciton not working :(
    --  awful.util.spawn("synclient TouchpadOff=
    --  $(synclient -l | grep -c 'TouchpadOff.*=.*0')")
    awful.key({ }, "XF86TouchpadToggle",
       function ()
           awful.util.spawn("synclient TouchpadOff=1")
           naughty.notify({ title = "Touchpad control",
           text = "Touchpad deactivated"})
       end),
    awful.key({"Shift" }, "XF86TouchpadToggle",
       function ()
           awful.util.spawn("synclient TouchpadOff=0")
           naughty.notify({ title = "Touchpad control",
           text = "Touchpad Activated"})
       end),
    -- Brightness
    awful.key({ }, "XF86MonBrightnessUp"  ,
        function () awful.util.spawn("xbacklight -set 100") end),
    awful.key({ }, "XF86MonBrightnessDown",
        function () awful.util.spawn("xbacklight -set 15") end),
